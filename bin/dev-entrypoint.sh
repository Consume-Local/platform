...
# 3: Define the tunneling methods:
# Tunnels up the backend hosts & ports so web_ui's ember-fastboot + cached-shoe
# can work from containers:
start_backend_tunnels() {
  # Tunnel up artanis:
  socat tcp-listen:4201,reuseaddr,fork tcp:host.docker.internal:4201 &
}
...
then
  ...
  if [ "$1" = "rails" ] && ([ "$2" = "server" ] || [ "$2" = "serve" ] || [ "$2" = "s" ])
  then
    echo "Starting backend web tunnels..."
    start_backend_tunnels
  fi
