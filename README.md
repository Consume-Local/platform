...
```bash
$ git clone git@github.com:Consume-Local/platform.git
...
That command will lift every service consume-local needs, such as the `rails server`, `postgres`, and `redis`.
...
In order to stop consume_local as a whole you can run:
...
You probably won't be working with a blank database, so once you are able to run consume_local you can restore the database, to do it, first stop all services:
...
```
% docker exec -ti consume_local_db_1 bash
```

This will open up a bash session in to the database container.

Up to this point we just need to download a database dump and copy under `consume-local/backups/`, this directory is mounted on the container, so you will be able to restore it with:

```
root@a3f695b39869:/# bin/restoredb consume_local_dev db/backups/<databaseDump>
```
